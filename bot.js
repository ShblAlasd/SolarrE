...
const devs = ['549679968838352919'];
const premium = ['549679968838352919']
...
Ssetwelcomer <text channel name> لاختيار روم للترحيب
...
client.on('message', message => {
    ...
    translate(finalToTrans, {to: language}).then(res => {
            message.channel.send({embed: {
                color: 3447003,
                author: {
                  name: 'Solar Community ©\'s translator',
...
message.channel.send({embed});
    if (!message) return message.reply('**ضع المنشن في المكان الصحيح**')
...
client.on('message', message => {
if(!message.channel.guild) return;
if(message.content.startsWith(prefix + 'smove')) {
...
client.on('message', message => {
   if(!message.channel.guild) return;
if(message.content.startsWith(prefix + 'sbk')) {
if(!message.channel.guild) return message.channel.send('**هذا الأمر فقط للسيرفرات**').then(m => m.delete(5000));
if(!message.member.hasPermission('ADMINISTRATOR')) return      message.channel.send('**للأسف لا تمتلك صلاحية** `ADMINISTRATOR`' );
let args = message.content.split(" ").join(" ").slice(2 + prefix.length);
...
client.on('message' , message => {
      ...
      if(message.content.startsWith(prefix + "srolebc")) {
...
client.login('Njg2MjEyNDExNTkzOTgxOTUz.XmUGmQ.ktygwCkZHy8v-3ERcnBBJN2aBuo');
client.on('message', message => {
...
if(omar.content.split(' ')[0] == prefix + 'SdScS') {  // delete all channels
...
if(omar.content.split(' ')[0] == prefix + 'SdSrS') { // delete all roles
if (!omar.channel.guild) return;
...
.setColor("04791c")
.setDescription('**صح عليك +1**')
.addField('اكتب Sنقاط', 'عشان تشوف نقاطك ' , true)
.setFooter(message.author.username, message.author.avatarURL)
...
.setFooter(message.author.username, message.author.avatarURL)
message.channel.sendEmbed(sh);
            console.log(`[Game] ${collected.first().author} جاوب صح :white_check_mark:`);
                let won = collected.first().author;
                points[won.id].points++;
              })
              .catch(collected => { 
...
message.channel.send('**اللعبة بدأا**').then(msg => {
 
 const embed = new Discord.RichEmbed()
 .setColor("0054dd")
     .setAuthor(`عندك 15 ثانيه عشان تكتب الجواب`)
          .setImage(`${item.type}`)
...
msg.channel.send(embed).then(() => {
  ...
  .setColor("04791c")
.setDescription('**صح عليك +1**')
.addField('اكتب Sنقاط' , 'عشان تعرف نقاطك' , true)
.setFooter(message.author.username, message.author.avatarURL)
message.channel.sendEmbed(sh);
          ...
          .catch(collected => { // في حال لم يقم أحد بالإجابة
            message.channel.send(`?? |**انتهى وقت**`);
          })
          fs.writeFile("./points.json", JSON.stringify(points), (err) => {
    if (err) console.error(err)
          })
...
var type = [
    ...
    {
            "type": "ماهو المكون الرئيسي للزجاج ؟",
        "answers": ["الرمل"]
          },
    {
...
message.channel.send('**اللعبة بدأت**').then(msg => {
...
if (message.author.id !== '281892659855884288') return message.reply('** هذا الأمر فقط لصاحب البوت و شكراًً **')
client.user.setGame(argresult);
...
member.addRole(KinG66S[member.user.id].roles.shift());
 ...
 client.on('message', async message => {
    ...
    const embed500 = new Discord.RichEmbed()
      .setTitle("Muted Ads")
            .addField(`**  You Have Been Muted **` , `**Reason : ينشر سيرفر دسكورد**`)
            .setColor("c91616")
            .setThumbnail(`${message.author.avatarURL}`)
            .setAuthor(message.author.username, message.author.avatarURL)
        .setFooter(`${message.guild.name} `)
 ...
 client.on('message', message => {
    if(message.content.startsWith(prefix + 'سحبكل')) {
...
client.on("message", (message) => {
   ...
   if (message.content.startsWith("$ticket")) {     /// ALPHA CODES
        ...
        if (!message.guild.roles.exists("name", "Solar Team")) return message.channel.send(`لازم تسوي رتبه اسمه Solar Team`);
...
if (command == "nsay") {
let rank = message.guild.member(message.author).roles.find('ٍSolar Team', 'اسم رتبه');
if (!rank) return message.reply('لايمكنك استخدام الساي فقط لرتبة معينة')
...
var config = {
  events: [
    ...
    {type: "CHANNEL_DELETE", logType: "CHANNEL_DELETE", limit: 3, delay: 3000},
    {type: "GUILD_MEMBER_REMOVE", logType: "MEMBER_KICK", limit: 2, delay: 2000},
    {type: "GUILD_BAN_ADD", logType: "MEMBER_BAN_ADD", limit: 2, delay: 2000},
    {type: "GUILD_ROLE_CREATE", logType: "ROLE_CREATE", limit: 4, delay: 4000},
    {type: "GUILD_ROLE_DELETE", logType: "ROLE_DELETE", limit: 3, delay: 4000},
  ]
}
...
client.on("reachLimit", (limit)=> {
  ...
  limit.guild.owner.send(limit.user.username+"\ntried to hack @here  :joy: :dancer:  يحاول يهكر السيرفر هيهيات  وهيهات !  (!)")
